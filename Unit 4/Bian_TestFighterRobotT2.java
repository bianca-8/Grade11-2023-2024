...
public class Bian_TestFighterRobotT2 extends FighterRobot {
	 ...
	 */
	public Bian_TestFighterRobotT2(City c, int a, int s, Direction d, int id, int health) {
	...
	public TurnRequest takeTurn(int energy, OppData[] data) {
		...
		if (energy >= this.getNumMoves() * BianBattleManager.MOVES_ENERGY_COST) {
			// even round
			if (round % 2 == 0) {
				turn = new TurnRequest(this.getAvenue() + this.getNumMoves(), this.getStreet(), -1, 0); // endAvenue, endStreet, fightID, numRounds
			}
			// odd round
			else {
				turn = new TurnRequest(this.getAvenue() - this.getNumMoves(), this.getStreet(), -1, 0); // endAvenue, endStreet, fightID, numRounds
