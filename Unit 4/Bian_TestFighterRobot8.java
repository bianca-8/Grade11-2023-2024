...
public class Bian_TestFighterRobot8 extends FighterRobot {
	private int health;
	private int round;
	private final int STRMIN = 0;
	private final int AVEMIN = 0;
	private final int ROUNDPENALTY = 3; // rounds until a penalty is given
	...
	public TurnRequest takeTurn(int energy, OppData[] data) {
		...
		final int PENALTYHEALTH = 2; // will move to avoid a penalty if above this amount of health
		int oppHealth = 0;
		TurnRequest turn;
		...
		// fighting player is dead or itself
			...
			// go to next id -- player is dead, is itself or has 2 people already at spot
		...
		if (fightID != -1 && (((this.round >= this.ROUNDPENALTY - 2 || dis / this.getNumMoves() <= this.ROUNDPENALTY - 2) && this.health >= PENALTYHEALTH) || this.health >= FIGHTHEALTH)) {
		...
		// lower than fightHealth amount of health
		else {
			// retreat
