...
public class Bian_TestFighterRobot2 extends FighterRobot {
	 ...
	 */
	public Bian_TestFighterRobot2(City c, int a, int s, Direction d, int id, int health) {
	...
	public TurnRequest takeTurn(int energy, OppData[] data) {
		...
		/*
		// find lowest health player
		selectSort(data, "health");
		// fighting player is not dead and is not itself
		if (data[index].getHealth() > 0 && data[index].getID() != this.getID()) {
			fightID = data[index].getID();
		}
		...
		*/
		...
		// Level 2 - fight player with lowest health/closest
	 ...
	 * @return sorted array
	 */
	private OppData [] selectSort(OppData [] list, String command) {
		// go through list
		for (int i = 0; i < list.length; i++) {
			...
			for (int j = i; j < list.length; j++) {
				// sort by health
				if (command == "health") {
					// health at j is lower than health at i, swap
					if (list[j].getHealth() < list[i].getHealth()) {
						OppData temp = list[i];
						list[i] = list[j];
						list[j] = temp;
					}
